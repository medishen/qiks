...
describe('TTLManager', () => {
  ...
  it('correctly identifies expired entries', async () => {
    const ttl = 100;
    const expiry = ttlManager.setTTL(ttl);
    const entry: CacheEntry<string> = { value: 'test', expiry };
  ...
  it('handles entries with no expiry set', () => {
    const entry: CacheEntry<string> = { value: 'test', expiry: null };
    expect(ttlManager.isExpired(entry)).to.be.false;
  });
  ...
  describe('Edge Cases', () => {
    it('handles undefined expiry gracefully', () => {
      const entry: CacheEntry<string> = { value: 'test' };
      expect(ttlManager.isExpired(entry)).to.be.false;
    });
    ...
    it('handles entries with null value but valid expiry', () => {
      const ttl = 1000;
      const expiry = ttlManager.setTTL(ttl);
      const entry: CacheEntry<null> = { value: null, expiry };
    ...
    it('handles entries with complex value and valid expiry', () => {
      const ttl = 1000;
      const expiry = ttlManager.setTTL(ttl);
      const entry: CacheEntry<object> = { value: { key: 'value' }, expiry };
